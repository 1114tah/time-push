...
def caihongpi():
    if (Whether_caihongpi!=False):
        try:
            ...
            params = urllib.parse.urlencode({'key':'66112750b61fc00bbd559384a412ac8b'})
...
def lucky():
    if ( Whether_lucky!=False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'66112750b61fc00bbd559384a412ac8b','astro':'白羊座'})
            headers = {'Content-type':'application/x-www-form-urlencoded'}
...
def tip():
    if (Whether_tip!=False):
        try:
            conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
            params = urllib.parse.urlencode({'key':'66112750b61fc00bbd559384a412ac8b','city':'洛阳市'})
            headers = {'Content-type':'application/x-www-form-urlencoded'}
